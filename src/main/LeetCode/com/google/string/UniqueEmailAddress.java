 ...
 * forwarded to the same address without dots in the local name.  For example, "alice.z@leetcode.com"
...
public class UniqueEmailAddress {
    // Time: O(N*m) m is length for each email, Space: O(N)
    public int numUniqueEmails(String[] emails) {
        Set<String> set = new HashSet<>();
        for (String email: emails) {
            set.add(simplify(email));
        }
        return set.size();
    }
    ...
    private String simplify(String email) {
        ...
        String localName = email.substring(0, delimiter);
        String domainName = email.substring(delimiter);
