...
public class FloydWarshallSolver {
  ...
  public FloydWarshallSolver(double[][] matrix) {
    n = matrix.length;
    dp = new double[n][n];
    // next[i][j]放的是从i到j走的所有步中的第一步，这样当我们traverse的时候是正序的，i=next[i][j] to go to next point
    // for ex. if from 0 to 6, we need traverse 0, 1, 2, 6, then next[0][6] = 1, next[1][6] = 2, next[2][6] = 6
  ...
  public void solve() {
    ...
    // 非常典型的DP思路 dp[k][i][j] = min(dp[k-1][i][j], dp[k-1][i][k] + dp[k-1][k][j])
    // 其中dp[k][i][j] denotes from i to j the shortest path if through nodes {0, 1, ... k-1, k}
    // 这里我们用了简化版 因为dp[k][i][j] only depends on dp[k-1][i][j], 但上面的写法更明了
    for (int k = 0; k < n; k++) {
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
